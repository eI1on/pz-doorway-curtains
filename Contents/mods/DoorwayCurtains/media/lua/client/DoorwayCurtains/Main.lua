...
function ISMoveableSpriteProps:canPlaceMoveableInternal(_character, _square, _item, _forceTypeObject)
    ...
    if self.isMoveable then
        ...
        if self.type == "WindowObject" then
            canPlace = self:hasFaces() and DoorwayCurtainsModule.getWallForFacing(_square, self.facing, "WallDoorframe");
            if canPlace then
        ...
        if not canPlace then
            return DoorwayCurtainsModule.oldCanPlaceMoveableInternal(self, _character, _square, _item, _forceTypeObject);
        end
    end
    return canPlace;
end
